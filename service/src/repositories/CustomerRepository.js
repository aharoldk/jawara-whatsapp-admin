...
class CustomerRepository {
  ...
  async searchByName(name) {
    return await Customer.find({ $text: { $search: name } }).limit(20);
  }
  ...
  async update(id, customerData) {
    return await Customer.findByIdAndUpdate(id, customerData, { new: true, runValidators: true });
  }
  ...
  async addTag(id, tag) {
    return await Customer.findByIdAndUpdate(id, { $addToSet: { tags: tag } }, { new: true });
  }

  async removeTag(id, tag) {
    return await Customer.findByIdAndUpdate(id, { $pull: { tags: tag } }, { new: true });
  }

  async updateLastContacted(id) {
    return await Customer.findByIdAndUpdate(id, { lastContactedAt: new Date() }, { new: true });
  }
